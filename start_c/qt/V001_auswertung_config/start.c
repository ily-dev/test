...
static void dump_pyconfig(PyConfig *config) {
    ...
    LOGP("isolated=%d", config->isolated);
    LOGP("use_environment=%d", config->use_environment);
    LOGP("site_import=%d", config->site_import);
    LOGP("install_signal_handlers=%d", config->install_signal_handlers);
    
    // 2. Pfade (wchar_t* -> char* konvertieren)
    wchar_t *wstr;
    char buffer[1024];
    
    wstr = config->home;
    LOGP("home=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->program_name;
    LOGP("program_name=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->executable;
    LOGP("executable=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->prefix;
    LOGP("prefix=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->exec_prefix;
    LOGP("exec_prefix=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->base_prefix;
    LOGP("base_prefix=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->base_exec_prefix;
    LOGP("base_exec_prefix=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    wstr = config->pythonpath_env;
    LOGP("pythonpath_env=%s", wstr ? PyUnicode_AsUTF8(wstr) : "(null)");
    
    // 3. Modul-Suchpfade
    PyWideStringList *paths = &config->module_search_paths;
    if (paths && paths->length > 0) {
        LOGP("module_search_paths_count=%zu", paths->length);
        for (size_t i = 0; i < paths->length && i < 20; i++) {
            if (paths->items[i]) {
                LOGP("  path[%zu]=%s", i, PyUnicode_AsUTF8(paths->items[i]));
            }
        }
    } else {
        LOGP("module_search_paths=(empty)");
    }
    
    // 4. Argv (PyWideStringList)
    PyWideStringList *argv_list = &config->argv;
    if (argv_list && argv_list->length > 0) {
        LOGP("argv_count=%zu", argv_list->length);
        for (size_t i = 0; i < argv_list->length && i < 20; i++) {
            if (argv_list->items[i]) {
                LOGP("  argv[%zu]=%s", i, PyUnicode_AsUTF8(argv_list->items[i]));
            }
        }
    } else {
        LOGP("argv_count=0");
    ...
    // 5. Weitere Flags
    LOGP("dev_mode=%d", config->dev_mode);
    LOGP("faulthandler=%d", config->faulthandler);
    LOGP("tracemalloc=%d", config->tracemalloc);
    LOGP("import_time=%d", config->import_time);
    LOGP("show_ref_count=%d", config->show_ref_count);
    LOGP("verbose=%d", config->verbose);
    
    LOGP("====== END PYCONFIG DUMP ======");
}
